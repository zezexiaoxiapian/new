...
def digital_twin_auction(p, target_positions, N, num_targets):
    ...
    for i in range(N):
        ...
        if assigned_task.size > 0:
            u[i] = 2 * (target_positions[assigned_task[0]] - p[i])  # Doubled the control signal strength
...
capture_distance = 1.0  # Distance within which drone slows down
...
while len(captured_targets) < num_targets:
    ...
    for t in range(num_targets):
        ...
        # Calculate errors for each agent w.r.t. each target
        e = [np.linalg.norm(p[i] - target_positions[t]) for i in range(N)]

        # 获取所有无人机的EKF协方差矩阵
        covariances = [drone.covariance for drone in ekf_drones]

        # 使用修改后的k-WTA策略
        winners = k_WTA(e, k, covariances)

        for i in range(N):
            if winners[i] == 1:
                direction = (target_positions[t] - p[i])
                distance_to_target = np.linalg.norm(direction)
                direction_normalized = direction / distance_to_target
                speed_factor = drone_speed
                if distance_to_target < capture_distance_threshold:
                    speed_factor *= distance_to_target / capture_distance  # Reduce speed as drone gets closer
                    if distance_to_target < 0.5:  # 例如，当距离小于0.5时，我们可以认为目标已被捕获
                        captured_targets.append(t)
                        print(f"Target {t+1} captured by Drone {i+1}!")

                u[i] = speed_factor * direction_normalized
